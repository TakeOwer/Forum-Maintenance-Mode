<?php
/**
 * Forum Maintenance Mode - phpBB extension
 *
 * @author     Salvo Cortesiano
 * @copyright  (c) 2026-08-11 20:00 CEST Salvo Cortesiano
 * @link       https://netshadows.de/ombra/
 * @license    GNU General Public License, version 2 (GPL-2.0)
 */

namespace salvocortesiano\maintenance\acp;

class main_module
{
	/** @var string */
	public $u_action;

	/** @var string */
	public $tpl_name;

	/** @var string */
	public $page_title;

	public function main($id, $mode)
	{
		global $phpbb_container, $request, $template, $user, $phpbb_root_path;

		/** @var \phpbb\config\config $config */
		$config = $phpbb_container->get('config');
		/** @var \phpbb\language\language $language */
		$language = $phpbb_container->get('language');
		/** @var \phpbb\controller\helper $controller_helper */
		$controller_helper = $phpbb_container->get('controller.helper');
		/** @var \salvocortesiano\maintenance\core\helper $agm */
		$agm = $phpbb_container->get('salvocortesiano.maintenance.helper');

		$language->add_lang('acp_maintenance', 'salvocortesiano/maintenance');

		$this->tpl_name = 'acp_agm_settings';
		$this->page_title = $language->lang('ACP_AGM_SETTINGS');

		$form_key = 'agm_settings';
		add_form_key($form_key);

		$errors = array();

		// Every button that has to do something must be listed here, otherwise
		// the form is simply redisplayed and the click appears to do nothing.
		$posted = false;

		foreach (array('submit', 'agm_toggle', 'agm_logo_upload', 'agm_logo_delete', 'agm_reopen', 'agm_load_lang', 'agm_reset_texts') as $button)
		{
			if ($request->is_set_post($button))
			{
				$posted = true;
				break;
			}
		}

		if ($posted)
		{
			// A confirmation box posts its own form, carrying its own token
			// instead of this page's form key. Asking for both would reject
			// every confirmed action, so the check is skipped in that case:
			// confirm_box() has already validated its own request.
			$confirmed = $request->is_set_post('confirm_key') || $request->is_set_post('confirm');

			if (!$confirmed && !check_form_key($form_key))
			{
				$errors[] = $language->lang('FORM_INVALID');
			}

			// Upload a logo and point the field at it
			if (empty($errors) && $request->is_set_post('agm_logo_upload'))
			{
				// The result says explicitly whether it is a path or a problem:
				// a message must never end up stored as if it were a file name.
				$uploaded = $this->store_logo($request, $language, $phpbb_root_path);

				if (isset($uploaded['path']))
				{
					$config->set('agm_logo_url', $uploaded['path']);

					trigger_error($language->lang('AGM_LOGO_UPLOADED') . adm_back_link($this->u_action));
				}

				$errors[] = $uploaded['error'];
			}

			// Drop the overrides of one language, so the shipped translation
			// comes back
			if (empty($errors) && $request->is_set_post('agm_reset_texts'))
			{
				$reset_lang = $request->variable('agm_text_lang', '');

				if (array_key_exists($reset_lang, $agm->available_languages()))
				{
					$agm->set_overrides($reset_lang, array());

					trigger_error($language->lang('AGM_TEXTS_RESET') . adm_back_link($this->u_action));
				}
			}

			// Delete the picture the field points at
			if (empty($errors) && $request->is_set_post('agm_logo_delete'))
			{
				if (confirm_box(true))
				{
					$removed = $this->remove_logo($language, $phpbb_root_path, (string) $config['agm_logo_url']);

					if (isset($removed['error']))
					{
						$errors[] = $removed['error'];
					}
					else
					{
						$config->set('agm_logo_url', '');

						trigger_error($language->lang('AGM_LOGO_DELETED') . adm_back_link($this->u_action));
					}
				}
				else
				{
					confirm_box(false, $language->lang('AGM_LOGO_DELETE_CONFIRM', (string) $config['agm_logo_url']), build_hidden_fields(array(
						'agm_logo_delete' => 1,
					)));
				}
			}

			// Reopen the board when it was left closed outside maintenance
			if (empty($errors) && $request->is_set_post('agm_reopen'))
			{
				$config->set('board_disable', 0);
				$config->set('agm_prev_board_disable', 0);
				$config->set('agm_board_governed', 0);

				trigger_error($language->lang('AGM_BOARD_REOPENED') . adm_back_link($this->u_action));
			}

			// Quick on/off button
			if (empty($errors) && $request->is_set_post('agm_toggle'))
			{
				if ($agm->is_enabled())
				{
					$agm->deactivate();
					$message = $language->lang('AGM_DEACTIVATED');
				}
				else
				{
					$agm->activate();
					$message = $language->lang('AGM_ACTIVATED');
				}

				trigger_error($message . adm_back_link($this->u_action));
			}

			if (empty($errors))
			{
				// The browser posts real timestamps (agm_ts_mode = 1) so the times
				// mean what the admin saw on screen, whatever timezone the
				// account is set to. Text parsing is only the no-JS fallback.
				if ($request->variable('agm_ts_mode', 0))
				{
					$start = (int) $request->variable('agm_start_ts', 0);
					$end   = (int) $request->variable('agm_end_ts', 0);
				}
				else
				{
					$start = $this->to_timestamp($request->variable('agm_start', '', true), $user);
					$end   = $this->to_timestamp($request->variable('agm_end', '', true), $user);
				}

				if ($start === false)
				{
					$errors[] = $language->lang('AGM_ERR_START');
					$start = 0;
				}

				if ($end === false)
				{
					$errors[] = $language->lang('AGM_ERR_END');
					$end = 0;
				}

				if ($start && $end && $end <= $start)
				{
					$errors[] = $language->lang('AGM_ERR_RANGE');
				}

				$email = trim($request->variable('agm_contact_email', '', true));

				if ($email !== '' && !preg_match('/^[^@\s]+@[^@\s]+\.[^@\s]+$/', $email))
				{
					$errors[] = $language->lang('AGM_ERR_EMAIL');
				}
			}

			// The Load button just switches which language the panel shows
			if ($request->is_set_post('agm_load_lang'))
			{
				$errors = array();
			}
			else if (empty($errors))
			{
				$config->set('agm_sync_board_disable', (int) $request->variable('agm_sync_board_disable', 0));
				$config->set('agm_show_to_admins', (int) $request->variable('agm_show_to_admins', 0));

				$was_enabled = $agm->is_enabled();
				$now_enabled = (bool) $request->variable('agm_enabled', 0);

				if ($now_enabled && !$was_enabled)
				{
					$agm->activate();
				}
				else if (!$now_enabled && $was_enabled)
				{
					$agm->deactivate();
				}
				else
				{
					$config->set('agm_enabled', $now_enabled ? 1 : 0);
				}

				$config->set('agm_notice', (int) $request->variable('agm_notice', 0));
				$config->set('agm_notice_days', max(0, min(365, (int) $request->variable('agm_notice_days', 0))));
				$config->set('agm_use_schedule', (int) $request->variable('agm_use_schedule', 0));
				$config->set('agm_auto_off', (int) $request->variable('agm_auto_off', 0));
				$config->set('agm_start', (int) $start);
				$config->set('agm_end', (int) $end);

				$config->set('agm_contact_email', $request->variable('agm_contact_email', '', true));
				$config->set('agm_contact_phone', $request->variable('agm_contact_phone', '', true));
				$config->set('agm_logo_url', $request->variable('agm_logo_url', '', true));
				$default_lang = $request->variable('agm_default_lang', 'it');
				$config->set('agm_default_lang', array_key_exists($default_lang, $agm->available_languages()) ? $default_lang : 'en');

				$config->set('agm_particles', (int) $request->variable('agm_particles', 0));
				$style = $request->variable('agm_particle_style', 'snow');
				$config->set('agm_particle_style', in_array($style, array('snow', 'stars', 'both'), true) ? $style : 'snow');
				$config->set('agm_show_gear', (int) $request->variable('agm_show_gear', 0));
				$config->set('agm_spin_gear', (int) $request->variable('agm_spin_gear', 0));
				$config->set('agm_countdown', (int) $request->variable('agm_countdown', 0));
				$config->set('agm_show_dates', (int) $request->variable('agm_show_dates', 0));
				$config->set('agm_progress_bar', (int) $request->variable('agm_progress_bar', 0));
				$config->set('agm_lang_switcher', (int) $request->variable('agm_lang_switcher', 0));
				$config->set('agm_show_admin_link', (int) $request->variable('agm_show_admin_link', 0));
				$config->set('agm_refresh', max(0, min(3600, (int) $request->variable('agm_refresh', 30))));

				foreach (array('agm_color_bg_start', 'agm_color_bg_end', 'agm_color_topbar', 'agm_color_accent', 'agm_color_text', 'agm_color_muted', 'agm_color_card', 'agm_color_cd_from', 'agm_color_cd_to', 'agm_color_prog_a', 'agm_color_prog_b') as $key)
				{
					$value = trim($request->variable($key, ''));

					if (preg_match('/^#[0-9a-fA-F]{3,8}$/', $value))
					{
						$config->set($key, $value);
					}
				}

				// Texts are saved for the language currently open in the panel;
				// an empty box means "use the wording from the language file".
				// The fields on screen belong to the language that was loaded, which
				// is not necessarily the one now picked in the drop down: saving
				// by the drop down would file one language's words under another.
				$edit_lang = $request->variable('agm_text_lang', '');

				if (array_key_exists($edit_lang, $agm->available_languages()))
				{
					$fields = array();

					foreach (array_keys(\salvocortesiano\maintenance\core\helper::overridable()) as $field)
					{
						$fields[$field] = $request->variable('agm_text_' . $field, '', true);
					}

					$agm->set_overrides($edit_lang, $fields);
				}

				trigger_error($language->lang('AGM_SETTINGS_SAVED') . adm_back_link($this->u_action));
			}
		}

		$notice_preview = $agm->get_notice();
		$colors = $agm->get_colors();
		$available = $agm->available_languages();
		$start = $agm->get_start();
		$end = $agm->get_end();

		// Which language the panel is editing: the one just picked, otherwise
		// the board default, otherwise the first one available.
		$edit = $request->is_set_post('agm_load_lang')
			? $request->variable('agm_edit_lang', '')
			: $request->variable('agm_text_lang', '');

		if (!isset($available[$edit]))
		{
			$edit = (string) $config['agm_default_lang'];
		}

		if (!isset($available[$edit]))
		{
			$keys = array_keys($available);
			$edit = (string) array_shift($keys);
		}

		$texts = $agm->page_texts($edit);

		foreach ($available as $iso => $info)
		{
			$template->assign_block_vars('agm_lang_option', array(
				'ISO'        => $iso,
				'NAME'       => $info['name'],
				'CODE'       => $info['code'],
				'S_SELECTED' => ($iso === (string) $config['agm_default_lang']),
				'S_EDITING'  => ($iso === $edit),
			));
		}

		// What the page will actually show, and why: each block on the
		// maintenance page has a condition, and a missing date is the usual
		// reason something does not appear.
		$check_countdown = (!empty($config['agm_countdown']) && $end > 0);
		$check_dates     = (!empty($config['agm_show_dates']) && ($start > 0 || $end > 0));
		$check_progress  = (!empty($config['agm_progress_bar']) && $start > 0 && $end > $start);
		$check_contact   = (trim((string) $config['agm_contact_email']) !== '' || trim((string) $config['agm_contact_phone']) !== '');

		$template->assign_vars(array(
			'AGM_EDIT_LANG'        => $edit,
			'AGM_EDIT_LANG_NAME'   => isset($available[$edit]['name']) ? $available[$edit]['name'] : $edit,
			'AGM_LANG_COUNT'       => count($available),
			'AGM_TEXT_TITLE'       => isset($texts['AGM_PAGE_TITLE']) ? $texts['AGM_PAGE_TITLE'] : '',
			'AGM_TEXT_SUBTITLE'    => isset($texts['AGM_PAGE_SUBTITLE']) ? $texts['AGM_PAGE_SUBTITLE'] : '',
			'AGM_TEXT_DESCRIPTION' => isset($texts['AGM_PAGE_DESCRIPTION']) ? $texts['AGM_PAGE_DESCRIPTION'] : '',
			'AGM_TEXT_FOOTER'      => isset($texts['AGM_PAGE_FOOTER']) ? $texts['AGM_PAGE_FOOTER'] : '',
			'AGM_TEXT_NOTICE'      => isset($texts['AGM_PAGE_NOTICE']) ? $texts['AGM_PAGE_NOTICE'] : '',

			'S_AGM_CHECK_COUNTDOWN' => $check_countdown,
			'S_AGM_CHECK_DATES'     => $check_dates,
			'S_AGM_CHECK_PROGRESS'  => $check_progress,
			'S_AGM_CHECK_CONTACT'   => $check_contact,

			'S_ERROR'	=> (bool) count($errors),
			'ERROR_MSG'	=> implode('<br />', $errors),
			'U_ACTION'	=> $this->u_action,
			'U_AGM_PREVIEW' => $controller_helper->route('salvocortesiano_maintenance_preview'),

			'AGM_IS_ENABLED'	=> $agm->is_enabled(),
			'AGM_IS_ACTIVE'		=> $agm->is_active(),
			'AGM_NOTICE_ON'		=> !empty($config['agm_notice']),
			'AGM_NOTICE_DAYS'	=> (int) $config['agm_notice_days'],
			'AGM_NOTICE_PREVIEW' => isset($notice_preview['text']) ? $notice_preview['text'] : '',
			'AGM_USE_SCHEDULE'	=> !empty($config['agm_use_schedule']),
			'AGM_AUTO_OFF'		=> !empty($config['agm_auto_off']),
			'AGM_SYNC_BOARD'	=> !empty($config['agm_sync_board_disable']),
			'AGM_SHOW_TO_ADMINS' => !empty($config['agm_show_to_admins']),
			'AGM_BOARD_DISABLED' => !empty($config['board_disable']),
			'AGM_BOARD_STUCK'	=> (!empty($config['board_disable']) && !$agm->is_active()),

			'AGM_START_TS'		=> $start,
			'AGM_END_TS'		=> $end,
			'AGM_START'			=> $this->to_input($start, $user),
			'AGM_END'			=> $this->to_input($end, $user),
			'AGM_START_HUMAN'	=> $start ? $user->format_date($start) : $language->lang('AGM_NOT_AVAILABLE'),
			'AGM_END_HUMAN'		=> $end ? $user->format_date($end) : $language->lang('AGM_NOT_AVAILABLE'),
			'AGM_DURATION'		=> ($start && $end && $end > $start) ? $this->format_duration($end - $start, $language) : $language->lang('AGM_NOT_AVAILABLE'),

			'AGM_CONTACT_EMAIL'	=> $config['agm_contact_email'],
			'AGM_CONTACT_PHONE'	=> $config['agm_contact_phone'],
			'AGM_LOGO_URL'		=> $config['agm_logo_url'],
			'AGM_LOGO_PREVIEW'	=> $agm->logo_preview(),
			'AGM_DEFAULT_LANG'	=> $config['agm_default_lang'],

			'AGM_PARTICLES'		=> !empty($config['agm_particles']),
			'AGM_STYLE_SNOW'	=> ((string) $config['agm_particle_style'] !== 'stars' && (string) $config['agm_particle_style'] !== 'both'),
			'AGM_STYLE_STARS'	=> ((string) $config['agm_particle_style'] === 'stars'),
			'AGM_STYLE_BOTH'	=> ((string) $config['agm_particle_style'] === 'both'),
			'AGM_SHOW_GEAR'		=> !empty($config['agm_show_gear']),
			'AGM_SPIN_GEAR'		=> !empty($config['agm_spin_gear']),
			'AGM_COUNTDOWN'		=> !empty($config['agm_countdown']),
			'AGM_SHOW_DATES'	=> !empty($config['agm_show_dates']),
			'AGM_PROGRESS_BAR'	=> !empty($config['agm_progress_bar']),
			'AGM_LANG_SWITCHER'	=> !empty($config['agm_lang_switcher']),
			'AGM_SHOW_ADMIN_LINK' => !empty($config['agm_show_admin_link']),
			'AGM_REFRESH'		=> (int) $config['agm_refresh'],

			'AGM_C_BG_START'	=> $colors['agm_color_bg_start'],
			'AGM_C_BG_END'		=> $colors['agm_color_bg_end'],
			'AGM_C_TOPBAR'		=> $colors['agm_color_topbar'],
			'AGM_C_ACCENT'		=> $colors['agm_color_accent'],
			'AGM_C_TEXT'		=> $colors['agm_color_text'],
			'AGM_C_MUTED'		=> $colors['agm_color_muted'],
			'AGM_C_CARD'		=> $colors['agm_color_card'],
			'AGM_C_CD_FROM'		=> $colors['agm_color_cd_from'],
			'AGM_C_CD_TO'		=> $colors['agm_color_cd_to'],
			'AGM_C_PROG_A'		=> $colors['agm_color_prog_a'],
			'AGM_C_PROG_B'		=> $colors['agm_color_prog_b'],

			// The browser colour picker only understands #rrggbb, so the
			// six-digit part and the alpha byte are passed separately. Doing it
			// here means the swatches are already right without any JavaScript.
			'AGM_C_CD_FROM_RGB'	=> $this->rgb_part($colors['agm_color_cd_from']),
			'AGM_C_CD_TO_RGB'	=> $this->rgb_part($colors['agm_color_cd_to']),
			'AGM_C_CARD_RGB'	=> $this->rgb_part($colors['agm_color_card']),
			'AGM_C_CD_FROM_A'	=> $this->alpha_part($colors['agm_color_cd_from']),
			'AGM_C_CD_TO_A'		=> $this->alpha_part($colors['agm_color_cd_to']),
			'AGM_C_CARD_A'		=> $this->alpha_part($colors['agm_color_card']),

			'AGM_LAST_ON'		=> !empty($config['agm_last_on']) ? $user->format_date((int) $config['agm_last_on']) : $language->lang('AGM_NOT_AVAILABLE'),
			'AGM_LAST_OFF'		=> !empty($config['agm_last_off']) ? $user->format_date((int) $config['agm_last_off']) : $language->lang('AGM_NOT_AVAILABLE'),
			'AGM_TOTAL_ON'		=> (int) $config['agm_total_activations'],
		));
	}

	/**
	 * '#rrggbbaa' -> '#rrggbb' for the browser colour picker.
	 */
	protected function rgb_part($value)
	{
		return preg_match('/^#([0-9a-fA-F]{6})/', (string) $value, $m) ? '#' . $m[1] : '#000000';
	}

	/**
	 * '#rrggbbaa' -> 0-255 alpha; fully opaque when no alpha is present.
	 */
	protected function alpha_part($value)
	{
		return preg_match('/^#[0-9a-fA-F]{6}([0-9a-fA-F]{2})$/', (string) $value, $m) ? hexdec($m[1]) : 255;
	}

	/**
	 * datetime-local value -> UNIX timestamp in the admin timezone.
	 *
	 * @return int|false 0 when empty, false when malformed
	 */
	protected function to_timestamp($value, \phpbb\user $user)
	{
		$value = trim($value);

		if ($value === '')
		{
			return 0;
		}

		$value = str_replace(' ', 'T', $value);

		if (strlen($value) === 19)
		{
			$value = substr($value, 0, 16);
		}

		$timezone = ($user->timezone instanceof \DateTimeZone) ? $user->timezone : new \DateTimeZone('UTC');
		$date = \DateTime::createFromFormat('Y-m-d\TH:i', $value, $timezone);

		if ($date === false)
		{
			return false;
		}

		return $date->getTimestamp();
	}

	/**
	 * UNIX timestamp -> datetime-local value in the admin timezone.
	 */
	protected function to_input($timestamp, \phpbb\user $user)
	{
		if (empty($timestamp))
		{
			return '';
		}

		$timezone = ($user->timezone instanceof \DateTimeZone) ? $user->timezone : new \DateTimeZone('UTC');
		$date = new \DateTime('@' . (int) $timestamp);
		$date->setTimezone($timezone);

		return $date->format('Y-m-d\TH:i');
	}

	/**
	 * Seconds -> "2 d 4 h 30 min".
	 */
	protected function format_duration($seconds, \phpbb\language\language $language)
	{
		$days = (int) floor($seconds / 86400);
		$hours = (int) floor($seconds % 86400 / 3600);
		$minutes = (int) floor($seconds % 3600 / 60);

		$parts = array();

		if ($days)
		{
			$parts[] = $days . ' ' . $language->lang('AGM_UNIT_DAYS');
		}

		if ($hours)
		{
			$parts[] = $hours . ' ' . $language->lang('AGM_UNIT_HOURS');
		}

		if ($minutes || empty($parts))
		{
			$parts[] = $minutes . ' ' . $language->lang('AGM_UNIT_MINUTES');
		}

		return implode(' ', $parts);
	}

	/**
	 * Take the picture the administrator picked and put it where the page can
	 * reach it.
	 *
	 * The file is accepted only when the image functions themselves recognise
	 * it as a PNG or a JPEG, and it is written under a name built here, never
	 * the one that came from the browser: an uploaded name is not to be
	 * trusted.
	 *
	 * @return string the relative path on success, or the error to show
	 */
	protected function store_logo($request, $language, $root_path)
	{
		$file = $request->file('agm_logo_file');

		if (empty($file['name']) || empty($file['tmp_name']))
		{
			return array('error' => $language->lang('AGM_ERR_UPLOAD_NONE'));
		}

		if (!empty($file['error']))
		{
			// The web server refused it before PHP could look at it, almost
			// always because it is larger than the server allows.
			return array('error' => $language->lang('AGM_ERR_UPLOAD_SIZE'));
		}

		// Two megabytes is plenty for a logo and keeps the folder tidy
		if ((int) $file['size'] > 2097152)
		{
			return array('error' => $language->lang('AGM_ERR_UPLOAD_SIZE'));
		}

		$info = @getimagesize($file['tmp_name']);
		$extensions = array(IMAGETYPE_PNG => 'png', IMAGETYPE_JPEG => 'jpg');

		if ($info === false || !isset($info[2]) || !isset($extensions[$info[2]]))
		{
			return array('error' => $language->lang('AGM_ERR_UPLOAD_TYPE'));
		}

		// phpBB's own images folder, resolved from the board root rather than
		// from wherever the script happens to be running.
		$dir = $root_path . 'images/';

		if (!is_dir($dir) || !is_writable($dir))
		{
			return array('error' => $language->lang('AGM_ERR_UPLOAD_DIR', $this->readable_path($dir)));
		}

		// The prefix keeps our uploads apart from the files phpBB ships, so
		// nothing of the board's own can ever be overwritten.
		$name = 'maintenance_logo_' . time() . '.' . $extensions[$info[2]];
		$target = $dir . $name;

		if (!@move_uploaded_file($file['tmp_name'], $target) || !file_exists($target))
		{
			return array('error' => $language->lang('AGM_ERR_UPLOAD_MOVE', $this->readable_path($dir)));
		}

		@chmod($target, 0644);

		return array('path' => 'images/' . $name);
	}

	/**
	 * Delete the picture the logo field points at.
	 *
	 * Only a path inside the board's own folders is ever touched, and only when
	 * it really is an image: the field can be typed by hand, so it is treated
	 * as untrusted. An address on another site is not ours to delete, so the
	 * field is simply cleared.
	 *
	 * @return array empty on success, or the reason it could not be done
	 */
	protected function remove_logo($language, $root_path, $stored)
	{
		$stored = trim($stored);

		if ($stored === '')
		{
			return array('error' => $language->lang('AGM_ERR_DELETE_NONE'));
		}

		// Somewhere else on the internet: nothing to delete here
		if (preg_match('#^(https?:)?//#i', $stored) || strpos($stored, 'data:') === 0)
		{
			return array();
		}

		$relative = ltrim(str_replace('\\', '/', $stored), '/');

		// A path that climbs out of the board is refused outright
		if (strpos($relative, '..') !== false || strpos($relative, "\0") !== false)
		{
			return array('error' => $language->lang('AGM_ERR_DELETE'));
		}

		if (!preg_match('/\.(png|jpe?g|gif|webp)$/i', $relative))
		{
			return array('error' => $language->lang('AGM_ERR_DELETE'));
		}

		$file = $root_path . $relative;
		$real = @realpath($file);
		$root = @realpath($root_path);

		// Already gone: clearing the field is enough
		if ($real === false || !is_file($real))
		{
			return array();
		}

		// The resolved file must still sit under the board, whatever the field said
		if ($root === false || strpos($real, $root) !== 0)
		{
			return array('error' => $language->lang('AGM_ERR_DELETE'));
		}

		if (!@unlink($real))
		{
			return array('error' => $language->lang('AGM_ERR_DELETE'));
		}

		return array();
	}

	/**
	 * The folder as it really is on disk, so an error message points at
	 * something the administrator can go and look at.
	 */
	protected function readable_path($dir)
	{
		$real = @realpath($dir);

		return ($real !== false) ? $real : $dir;
	}
}
